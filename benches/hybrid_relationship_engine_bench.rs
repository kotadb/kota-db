    ...
    group.bench_function("init_with_10k_symbols", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                let engine = HybridRelationshipEngine::new(
                    black_box(db_path),
                    black_box(config),
                ).await.unwrap();
...
fn bench_symbol_lookup_performance(c: &mut Criterion) {
    ...
    for &count in &symbol_counts {
        ...
        group.bench_with_input(
            BenchmarkId::new("uuid_lookup", count),
            &count,
            |b, _| {
        ...
        group.bench_with_input(
            BenchmarkId::new("name_lookup", count),
            &count,
            |b, _| {
...
fn bench_relationship_queries(c: &mut Criterion) {
    ...
    // Create test symbol database
    let symbol_db_path = db_path.join("symbols.kota");
    let symbol_names = rt.block_on(async {
        create_test_symbol_database(&symbol_db_path, 20_000).unwrap()
    });
    
    let engine = rt.block_on(async {
        let config = RelationshipQueryConfig::default();
        HybridRelationshipEngine::new(db_path, config).await.unwrap()
    });
...
fn bench_query_latency_requirement(c: &mut Criterion) {
    ...
    let symbol_db_path = db_path.join("symbols.kota");
    let symbol_names = rt.block_on(async {
        create_test_symbol_database(&symbol_db_path, 30_000).unwrap()
    });
        ...
        b.iter(|| {
            rt.block_on(async {
                ...
                assert!(elapsed.as_millis() < 10, 
                    "Query took {:?}, exceeding 10ms requirement", elapsed);
