...
impl PrimaryIndex {
    ...
    async fn load_existing_index(&self) -> Result<()> {
        ...
        if btree_path.exists() {
            let btree_content = fs::read_to_string(&btree_path).await
                .with_context(|| format!("Failed to read B+ tree data: {}", btree_path.display()))?;
            ...
            for (id_str, path_str) in raw_mappings {
                ...
                btree_root = btree::insert_into_tree(btree_root, doc_id, validated_path)
                    .with_context(|| format!("Failed to insert into B+ tree: {} -> {}", id_str, path_str))?;
            }
    ...
    async fn save_mappings(&self) -> Result<()> {
        ...
        let all_pairs = btree::extract_all_pairs(&btree_root);
        ...
        fs::write(&btree_path, content).await
    ...
    async fn update_metadata(&self, document_count_delta: i32) -> Result<()> {
        ...
        if document_count_delta < 0 {
            ...
            if metadata.document_count < decrease {
                bail!("Document count would go negative: {} - {}", metadata.document_count, decrease);
            }
    ...
    fn validate_insert_preconditions(key: &ValidatedDocumentId, value: &ValidatedPath) -> Result<()> {
        // Key validation
    ...
    async fn validate_insert_postcondition(&self, key: &ValidatedDocumentId, value: &ValidatedPath) -> Result<()> {
        let btree_root = self.btree_root.read().await;
        ...
        match btree::search_in_tree(&btree_root, key) {
            ...
            None => {
                bail!("Insert postcondition failed: Key {} not found after insertion", key.as_uuid());
            }
        }
    }
    ...
    async fn validate_delete_postcondition(&self, key: &ValidatedDocumentId) -> Result<()> {
        ...
        if btree::search_in_tree(&btree_root, key).is_some() {
            bail!("Delete postcondition failed: Key {} still exists after deletion", key.as_uuid());
        }
...
impl Index for PrimaryIndex {
    ...
    async fn search(&self, query: &Query) -> Result<Vec<Self::Value>> {
        ...
        // Extract just the paths for results
        let mut results: Vec<ValidatedPath> = all_pairs.into_iter()
            .map(|(_, path)| path)
            .collect();
