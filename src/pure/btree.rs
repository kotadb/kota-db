...
pub fn insert_into_tree(
    ...
    if let Some((left_child, median_key, right_child)) = needs_new_root {
        ...
        if let BTreeNode::Internal { ref mut keys, ref mut children } = new_internal_root {
...
fn insert_recursive(
    ...
    key: ValidatedDocumentId,
    value: ValidatedPath,
) -> Result<(Box<BTreeNode>, Option<(Box<BTreeNode>, ValidatedDocumentId, Box<BTreeNode>)>)> {
    match node.as_mut() {
        BTreeNode::Leaf { .. } => {
            // Insert into leaf
            *node = Box::new(insert_key_value_in_leaf(*node, key, value)?);
            
            // Check if split needed
            if node.needs_split() {
                let (left, median, right) = split_leaf_node(*node)?;
                Ok((Box::new(left), Some((Box::new(left), median, Box::new(right)))))
            } else {
                Ok((node, None))
            }
        }
...
fn merge_with_left_sibling(
    ...
    match (left_node.as_mut(), right_node.as_ref()) {
        (BTreeNode::Leaf { keys: left_keys, values: left_values, next_leaf },
         BTreeNode::Leaf { keys: right_keys, values: right_values, next_leaf: right_next }) => {
