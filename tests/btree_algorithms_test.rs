...
mod btree_tree_operations_tests {
    ...
    fn test_btree_deletion_from_leaf() -> Result<()> {
        ...
        let keys: Vec<_> = (0..5).map(|_| ValidatedDocumentId::from_uuid(Uuid::new_v4()).unwrap()).collect();
    ...
    fn test_btree_deletion_causing_redistribution() -> Result<()> {
        ...
        let keys: Vec<_> = (0..20).map(|_| ValidatedDocumentId::from_uuid(Uuid::new_v4()).unwrap()).collect();
        for (i, key) in keys.iter().enumerate() {
    ...
    fn test_btree_deletion_causing_merge() -> Result<()> {
        ...
        let keys: Vec<_> = (0..30).map(|_| ValidatedDocumentId::from_uuid(Uuid::new_v4()).unwrap()).collect();
        for (i, key) in keys.iter().enumerate() {
