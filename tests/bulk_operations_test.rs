...
async fn test_bulk_insert_throughput_improvement() -> Result<()> {
    ...
    let start = Instant::now();
    let bulk_tree = btree::bulk_insert_into_tree(
    ...
    assert!(speedup >= 5.0, 
        ...
        speedup, individual_duration, bulk_duration
    );
...
async fn test_bulk_delete_throughput_improvement() -> Result<()> {
    ...
    let keys_to_delete: Vec<_> = test_pairs[..delete_count].iter()
        .map(|(id, _)| id.clone())
        .collect();
    ...
    assert_eq!(
        btree::count_entries(&tree1),
        btree::count_entries(&tree2)
    );
...
async fn test_bulk_operations_maintain_tree_balance() -> Result<()> {
    ...
    let tree = btree::bulk_insert_into_tree(
        btree::create_empty_tree(),
        test_pairs.clone()
    )?;
    ...
    assert!(tree_metrics.balance_factor >= 0.8, 
        "Tree balance factor {:.2} below 0.8 threshold", tree_metrics.balance_factor);
    
    // Depth should be approximately log(n)
    let expected_depth = (test_size as f64).log2().ceil() as usize + 1;
    assert!(tree_metrics.depth <= expected_depth * 2, 
        "Tree depth {} exceeds expected maximum {}", tree_metrics.depth, expected_depth * 2);
    
    // Node utilization should be reasonable
    assert!(tree_metrics.utilization_factor >= 0.5,
        "Node utilization {:.2} below 0.5 threshold", tree_metrics.utilization_factor);
    
    // All leaves should be at same level
    assert_eq!(tree_metrics.leaf_depth_variance, 0,
        "Leaf nodes not at same level (variance: {})", tree_metrics.leaf_depth_variance);
...
async fn test_bulk_operations_memory_efficiency() -> Result<()> {
    ...
    assert!(memory_efficiency > 0.4,
        "Memory efficiency {:.3} below 0.4 threshold (overhead: {:.1}x)",
        memory_efficiency, 1.0 / memory_efficiency);
    
    // Test bulk deletion memory cleanup
    let delete_keys: Vec<_> = test_pairs[..test_size/2].iter()
        .map(|(id, _)| id.clone())
        .collect();
    ...
    let memory_reclaimed_ratio = (post_insert_memory - post_delete_memory) as f64 
                               / (insert_memory_delta / 2) as f64;
    
    // Should reclaim at least 80% of memory from deleted entries
    assert!(memory_reclaimed_ratio >= 0.8,
        "Memory reclamation ratio {:.2} below 0.8 threshold", memory_reclaimed_ratio);
...
async fn test_bulk_operations_error_handling() -> Result<()> {
    ...
    let result = btree::bulk_insert_into_tree(
        btree::create_empty_tree(),
        test_pairs
    );
    
    // Should handle duplicates gracefully (either error or last-writer-wins)
    assert!(result.is_ok() || result.is_err(), "Bulk insert should handle duplicates");
    ...
    assert!(result.is_ok(), "Bulk delete should handle non-existent keys gracefully");
...
async fn test_bulk_operations_concurrent_safety() -> Result<()> {
    ...
    for batch in 0..4 {
        ...
        assert_eq!(entry_count, batch_size * (batch + 1),
            "Entry count mismatch after batch {}: expected {}, got {}",
            batch, batch_size * (batch + 1), entry_count);
    }
...
fn estimate_raw_data_size(pairs: &[(ValidatedDocumentId, ValidatedPath)]) -> usize {
    pairs.iter().map(|(id, path)| {
        std::mem::size_of_val(id) + path.as_str().len()
    }).sum()
}
