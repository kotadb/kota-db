...
fn compare_insertion_performance(sizes: &[usize]) {
    ...
    for &size in sizes {
        ...
        for i in 0..size {
            btree_idx = btree::insert_into_tree(btree_idx, keys[i].clone(), paths[i].clone()).unwrap();
        }
...
fn test_worst_case_scenarios() {
    ...
    println!("  B+Tree: {:?} (traverses ~{} levels)", btree_worst, (size as f64).log2() as u32);
    println!("  Speedup: {:.2}x", linear_worst.as_nanos() as f64 / btree_worst.as_nanos() as f64);
...
fn test_performance_at_scale() {
    ...
    println!("\nTheoretical linear search would take ~{:.0}μs average", theoretical_linear_us);
    println!("B+ tree is ~{:.0}x faster!", theoretical_linear_us / avg_search.as_micros() as f64);
}