...
async fn test_concurrent_reads_linear_scaling() -> Result<()> {
    ...
    for thread_count in [1, 2, 4, 8] {
        ...
        println!("Threads: {}, Duration: {:?}, Ops/sec: {:.0}", 
...
async fn test_concurrent_writes_isolation() -> Result<()> {
    ...
    for writer_id in 0..writers_count {
        ...
        let handle = task::spawn(async move {
            ...
            for i in 0..writes_per_writer {
                ...
                let path = ValidatedPath::new(&format!("/concurrent/writer_{}_{}.md", writer_id, i)).unwrap();
                
                // Acquire write lock and insert
                {
                    let mut tree_guard = tree_ref.write().unwrap();
                    *tree_guard = btree::insert_into_tree(tree_guard.clone(), id.clone(), path).unwrap();
                }
    ...
    assert_eq!(tree_size, writers_count * writes_per_writer,
        "Expected {} entries, found {}", writers_count * writes_per_writer, tree_size);
    
    // Verify all keys are searchable
    for key in &all_keys {
        assert!(btree::search_in_tree(&*final_tree, key).is_some(),
    ...
    sorted_keys.sort();
    sorted_keys.dedup();
    assert_eq!(sorted_keys.len(), all_keys.len(),
...
async fn test_mixed_read_write_workload() -> Result<()> {
    ...
    for reader_id in 0..readers_count {
        ...
        let handle = task::spawn(async move {
            ...
            println!("Reader {}: {} successful reads", reader_id, successful_reads);
            successful_reads
        });
    ...
    for writer_id in 0..writers_count {
        ...
        let handle = task::spawn(async move {
            ...
            for i in 0..operations_per_thread {
                let id = ValidatedDocumentId::from_uuid(Uuid::new_v4()).unwrap();
                let path = ValidatedPath::new(&format!("/mixed/writer_{}_{}.md", writer_id, i)).unwrap();
            ...
            println!("Writer {}: {} successful writes", writer_id, successful_writes);
            successful_writes
        });
    ...
    println!("Mixed workload: {} ops in {:?} ({:.0} ops/sec)", 
    ...
    assert_eq!(tree_size, final_keys.len(),
        "Tree size {} doesn't match key count {}", tree_size, final_keys.len());
    
    // Performance requirement: should handle at least 1000 ops/sec
    assert!(ops_per_second >= 1000.0,
        "Mixed workload performance {:.0} ops/sec below 1000 ops/sec requirement", ops_per_second);
...
async fn test_deadlock_prevention() -> Result<()> {
    ...
    for thread_id in 0..thread_count {
        ...
        let handle = task::spawn(async move {
            for i in 0..operations_per_thread {
                ...
                let path1 = ValidatedPath::new(&format!("/deadlock/t{}_op{}_tree1.md", thread_id, i)).unwrap();
                let path2 = ValidatedPath::new(&format!("/deadlock/t{}_op{}_tree2.md", thread_id, i)).unwrap();
    ...
    println!("✅ Deadlock prevention test passed: {} operations completed without deadlock", 
...
async fn test_lock_free_read_optimization() -> Result<()> {
    ...
    println!("Lock-free read baseline: {} reads/sec with {} threads", 
             reads_per_second as u64, read_threads);
    
    // Performance baseline: should achieve at least 100k reads/sec
    assert!(reads_per_second >= 100_000.0,
        "Read performance {:.0} reads/sec below 100k baseline", reads_per_second);
    
    // All reads should succeed (100% hit rate)
    assert_eq!(total_hits, read_threads * reads_per_thread,
        "Expected {} hits, got {}", read_threads * reads_per_thread, total_hits);
...
async fn test_write_batching_optimization() -> Result<()> {
        ...
        let handle = task::spawn(async move {
            for batch_id in 0..batches_per_writer {
                ...
                for i in 0..batch_size {
                    let id = ValidatedDocumentId::from_uuid(Uuid::new_v4()).unwrap();
                    let path = ValidatedPath::new(&format!("/batch/w{}_b{}_i{}.md", writer_id, batch_id, i)).unwrap();
    ...
    assert_eq!(final_size, total_writes,
        "Expected {} entries, found {}", total_writes, final_size);
    ...
    assert!(writes_per_second >= 10_000.0,
        "Batched write performance {:.0} writes/sec below 10k baseline", writes_per_second);
