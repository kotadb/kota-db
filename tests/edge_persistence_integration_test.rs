...
async fn test_complete_edge_persistence_pipeline() -> Result<()> {
    ...
    let graph_storage = NativeGraphStorage::new(
        graph_path.clone(),
    ...
    let new_file_storage = create_file_storage(
        storage_path.to_str().unwrap(),
        Some(100),
    ).await?;
    
    let _new_graph_storage = NativeGraphStorage::new(
        graph_path,
        GraphStorageConfig::default()
    ).await?;
    
    let _new_symbol_storage = SymbolStorage::with_graph_storage(
        Box::new(new_file_storage),
        Box::new(_new_graph_storage),
    ).await?;
...
async fn test_batch_rollback_on_failure() -> Result<()> {
    ...
    match result {
        Ok(()) => {
            ...
            assert!(new_relationships >= initial_relationships, "Should maintain or increase relationships");
        }
        Err(_) => {
            // If it failed, verify we didn't corrupt existing data
            let final_symbol_count = symbol_storage.get_stats().total_symbols;
            assert_eq!(final_symbol_count, initial_symbol_count, "Symbol count should remain stable on failure");
        }
    }
