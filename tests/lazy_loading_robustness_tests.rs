use anyhow::Result;
use kotadb::{create_primary_index, create_trigram_index, Index, QueryBuilder, ValidatedDocumentId, ValidatedPath};
use std::sync::Arc;
...
async fn test_concurrent_trigram_index_loading() -> Result<()> {
    ...
    for (thread_id, duration, count) in &results {
        assert_eq!(*count, expected_count, "Thread {} found different result count", thread_id);
        // Even concurrent loads should be reasonably fast
        assert!(duration.as_millis() < 2000, "Thread {} took too long: {:?}", thread_id, duration);
    }

    // At least one thread should have found results
    assert!(expected_count > 0, "No documents were found");
    
    println!("Concurrent loading test passed: {} threads, {} results each", 
...
async fn test_trigram_index_load_failure_protection() -> Result<()> {
    ...
    assert!(first_duration.as_millis() > 10, "Should take some time to fail initially");
    ...
    assert!(second_duration.as_millis() < 50, "Second failure should be immediate, took {:?}", second_duration);
    
    // Error messages should indicate retry protection
    let err_msg = result2.unwrap_err().to_string();
    assert!(err_msg.contains("previously failed") || err_msg.contains("retry is disabled"), 
            "Error should indicate retry protection: {}", err_msg);
...
async fn test_primary_index_load_failure_protection() -> Result<()> {
    ...
    assert!(result2.is_err(), "Expected second search to fail");
    assert!(second_duration < first_duration, "Second failure should be faster than first");
    assert!(second_duration.as_millis() < 50, "Second failure should be very fast");
...
async fn test_loading_state_transitions() -> Result<()> {
    ...
    // Create valid index
    {
        ...
        index.insert_with_content(doc_id, path, b"test content").await?;
        index.flush().await?;
    }
    ...
    assert!(result.len() > 0, "Should find test document");
    ...
    assert!(duration.as_millis() < 10, "Loaded index search should be very fast");
...
async fn test_memory_usage_awareness() -> Result<()> {
    ...
    // Create a reasonably large index
    {
        ...
        for i in 0..100 {
            ...
            index.insert_with_content(doc_id, path, content.as_bytes()).await?;
        }
        index.flush().await?;
    }
    ...
    assert!(results.len() > 0, "Should find documents");
    ...
    assert!(load_time.as_millis() < 5000, "Large index should still load in reasonable time: {:?}", load_time);
    
    println!("Large index test: {} documents found, loaded in {:?}", results.len(), load_time);
