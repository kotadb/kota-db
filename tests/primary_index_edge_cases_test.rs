...
mod primary_index_edge_cases {
    ...
    async fn create_test_index() -> Result<impl Index<Key = ValidatedDocumentId, Value = ValidatedPath>> {
    ...
    async fn test_index_with_extremely_long_paths() -> Result<()> {
        ...
        let long_path = format!("/edge/{}/{}/{}.md", long_component, long_component, long_component);
    ...
    async fn test_index_many_small_operations() -> Result<()> {
        ...
        assert!(results.len() > 6000 && results.len() < 7000, 
                "Unexpected result count: {}", results.len());
...
mod primary_index_adversarial_tests {
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                for j in 0..100 {
                    ...
                    let doc_path = ValidatedPath::new(&format!("/adversarial/writer_{}_{}.md", i, j)).unwrap();
    ...
    async fn test_index_memory_pressure() -> Result<()> {
        ...
        for i in 0..1000 {
            let doc_id = ValidatedDocumentId::from_uuid(Uuid::new_v4())?;
            let doc_path = ValidatedPath::new(&format!("/adversarial/memory_pressure_{:06}.md", i))?;
    ...
    async fn test_index_pathological_key_distribution() -> Result<()> {
        ...
        assert!(duration.as_millis() < 100, "Search too slow with pathological keys: {:?}", duration);
