...
mod primary_index_performance_tests {
    ...
    async fn test_primary_index_insert_performance() -> Result<()> {
        ...
        assert!(avg_per_insert.as_millis() < 5, "Insert too slow: {:?} per operation", avg_per_insert);
    ...
    async fn test_primary_index_search_performance() -> Result<()> {
        ...
        assert!(avg_per_search.as_millis() < 1, "Search too slow: {:?} per operation", avg_per_search);
