...
async fn test_symbol_search_performance() -> Result<()> {
    ...
    let storage =
        kotadb::file_storage::create_file_storage(temp_path.to_str().unwrap(), Some(1000)).await?;
...
async fn test_large_symbol_dataset_performance() -> Result<()> {
    ...
    let storage =
        kotadb::file_storage::create_file_storage(temp_path.to_str().unwrap(), Some(10000)).await?;
